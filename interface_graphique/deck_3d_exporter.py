...
from labware_to_scad import generate_scad_placed
...
def find_openscad() -> str | None:
    """Retourne le chemin vers openscad(.exe) ou None si introuvable."""
    return C.OPENSCAD_EXECUTABLE or shutil.which("openscad")


def find_blender() -> str | None:
    """Retourne le chemin vers blender(.exe) ou None si introuvable."""
    return C.BLENDER_EXECUTABLE or shutil.which("blender")
...
def _canvas_to_jubilee_coords(
    ...
    Returns:
        (x_mm, y_mm, length_mm, width_mm) où :
          - (x_mm, y_mm) est le coin d'ancrage Jubilee (X vertical, Y horizontal)
          - length_mm est l'extension selon Jubilee-X
          - width_mm est l'extension selon Jubilee-Y
    """
    x0_px, y0_px, x1_px, y1_px = canvas.coords(canvas_plateau)
    scale_x = C.PLATEAU_W_MM / (x1_px - x0_px)
    scale_y = C.PLATEAU_H_MM / (y1_px - y0_px)

    ox1, oy1, ox2, oy2 = canvas.coords(obj.id)
    # Mapping identique à exporter.export_layout():
    #   canvas X  → Jubilee Y
    #   canvas Y  → Jubilee X
    y_mm      = (ox1 - x0_px) * scale_x
