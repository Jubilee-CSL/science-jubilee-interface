...
from jubilee_interface import app_paths, exporter, theme
...
class WorkspaceTab(ctk.CTkFrame):
    ...
    def _restore_labwares(self, data: dict):
        from jubilee_interface.coords import (
            MachineBBox,
            PlateauFrame,
            machine_bbox_to_canvas_topleft_px,
        )
        frame = PlateauFrame.from_canvas(self.canvas, self.canvas_plateau)

        for slot in data.get("slots", {}).values():
            if not slot.get("has_labware"):
                continue
            bbox = MachineBBox(
                x_mm=float(slot["coordinates"][0]),
                y_mm=float(slot["coordinates"][1]),
                length_mm=float(slot.get("length", 0.0)),
                width_mm=float(slot.get("width", 0.0)),
            )
            x_px, y_px = machine_bbox_to_canvas_topleft_px(bbox, frame)
            name = next(
